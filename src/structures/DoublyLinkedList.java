...
/**
 * Partial implementation of the List interface based on doubly-linked nodes with dummy nodes at the
 * head and tail. This sample code includes a complete implementation of the ListIterator.
 * <p>
 * Overridden versions of add(item), add(pos, item), contains(obj) and get(pos), are also provided
 * as examples.
 */
public class DoublyLinkedList<E> extends AbstractSequentialList<E> {
	...
	/**
	 * Inserts newNode into the list after current without updating size. Precondition: current !=
	 * null, newNode != null
	 */
	...
	/**
	 * Returns the Node whose index is pos, which will be head if pos = -1 and tail if pos = size
