...
public class Median_of_Two_Sorted_Arrays {
	...
	public double findMedianSortedArrays2(int[] nums1, int[] nums2) {
		int n1 = nums1.length;
		int n2 = nums2.length;
		if (n1 > n2) return findMedianSortedArrays2(nums2, nums1); // nums1 is the shorter one, find pos in shorter one
		boolean odd = ((n1 + n2) & 1) == 1;
		int l = 0, r = n1;
		int midAB = (n1 + n2 + 1) / 2;
		while (l <= r) {
			int cut1 = l + (r - l + 1) / 2;
			int cut2 = midAB - cut1;
			if ((cut1 == 0 || cut2 == n2 || nums1[cut1 - 1] <= nums2[cut2]) &&
					(cut1 == n1 || cut2 == 0 || nums1[cut1] >= nums2[cut2 - 1])) {
				double left;
				if (cut1 == 0) left = nums2[cut2 - 1];
				else if (cut2 == 0) left = nums1[cut1 - 1];
				else left = Math.max(nums1[cut1 - 1], nums2[cut2 - 1]);
				if (odd) return left;
				double right;
				if (cut1 == n1) right = nums2[cut2];
				else if (cut2 == n2) right = nums1[cut1];
				else right = Math.min(nums1[cut1], nums2[cut2]);
				return (left + right) / 2.0;
			} else if (nums1[cut1 - 1] > nums2[cut2]) {
				r = cut1 - 1;
			} else if (nums1[cut1] < nums2[cut2 - 1]) {
				l = cut1 + 1;
			}
		}
